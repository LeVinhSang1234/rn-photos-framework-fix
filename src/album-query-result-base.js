...
export default class AlbumQueryResultBase extends EventEmitter {
  ...
  sortAlbumsByTypeObject(typeArray) {
    ...
    newAlbumArray.sort((albumOne, albumTwo) => {
      let albumOneWeight = this.getSortWeigth(albumOne, typeArray);
      let albumTwoWeight = this.getSortWeigth(albumTwo, typeArray);
      return albumOneWeight > albumTwoWeight
        ? -1
        : albumOneWeight === albumTwoWeight
        ? albumOne.title.localeCompare(albumTwo.title)
        : 1;
    });
  ...
  getSortWeigth(albumObj, typeArray) {
    return typeArray.reduce((weight, typeObj, index) => {
      if (
        typeObj.type === albumObj.type &&
        typeObj.subType == albumObj.subType
      ) {
  ...
  onChange(changeHandler) {
    const listener = this.addListener("onChange", changeHandler);
